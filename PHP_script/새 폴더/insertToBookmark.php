...
function check_validity(){
    ...
    for ( $i = 0,  $size = count($checkedOffering); $i < $size; ++$i){
        ...
        $sql2 = "select count(bm_id) as count from bookmark_$memberID where bm_bmf_id = '$match_bmf_id' and bm_match_id ='$selected_id' and bm_from = 1 ";
...
if(check_validity()){
    for ( $i = 0,  $size = count($checkedOffering); $i < $size; ++$i){
        $selected_id = $checkedOffering[$i]['wr_id'];
        $sql3= "Insert into bookmark_$memberID set bm_match_id = '$selected_id', bm_date = '".G5_TIME_YMDHIS."', bm_from = 1, bm_bmf_id ='$match_bmf_id'";
