 ...
 mysqli_set_charset($con,"utf8");
    ...
    if($level =='emp'){
        if($selectedSegment=='임대'){
            $sql_query = "select * from g5_write_$boss where wr_office_permission = 2 AND wr_sale_type = 1 AND wr_sold_out != 1 ";
        }
        else if($selectedSegment=='매매'){
            $sql_query = "select * from g5_write_$boss where wr_office_permission = 2 AND wr_sale_type = 2 AND wr_sold_out != 1 ";
        }
        else{
            $sql_query = "select * from g5_write_$boss where wr_office_permission = 2 AND wr_sold_out = 1 ";
        }
    }
    else{
        if($selectedSegment=='임대'){
            $sql_query = "select * from g5_write_$memberID where wr_office_permission !='' AND wr_sale_type = 1 AND wr_sold_out != 1 ";
        }
        else if($selectedSegment=='매매'){
            $sql_query = "select * from g5_write_$memberID where wr_office_permission !='' AND wr_sale_type = 2 AND wr_sold_out != 1 ";
        }
        else{
            $sql_query = "select * from g5_write_$memberID where wr_office_permission !='' AND wr_sold_out = 1 ";
