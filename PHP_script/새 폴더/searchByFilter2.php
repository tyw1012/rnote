...
header("Content-Type: text/html; charset=UTF-8");
 ...
 mysqli_set_charset($con,"utf8");
    ...
    if($level == 'emp'){
        ...
        if($selectedIndex==0){
            $sql_query = "select * from g5_write_$memberID where wr_office_permission='' AND wr_writer = '$memberName' and wr_sold_out != 1 AND wr_sale_type = 1 ";
        }
        else if($selectedIndex==1){
            $sql_query = "select * from g5_write_$memberID where wr_office_permission='' AND wr_writer = '$memberName' and wr_sold_out != 1 AND wr_sale_type = 2 ";
        }
        else{
            $sql_query = "select * from g5_write_$memberID where wr_office_permission='' AND wr_writer = '$memberName' AND wr_sold_out = 1 ";
        }
    }
