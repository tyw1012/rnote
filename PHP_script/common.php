...
if (isset($_REQUEST['mobile_main'])) {
    $wr_id = (int)$_REQUEST['mobile_main'];
} else {
    $mobile_main = 0;
}
...
if (isset($_REQUEST['gr_member_id'])) {
    if (!is_array($_REQUEST['gr_member_id'])) {
        $gr_member_id= preg_replace('/[^a-z0-9_]/i', '', trim($_REQUEST['gr_member_id']));
    }
} else {
    $gr_member_id = '';
}
...
$gr_info = sql_fetch(" select * from {$g5['group_table']} a,  {$g5['group_member_table']} b  where a.gr_1 = 'gr_admin' and b.gr_id = '$member[mb_id]' and a.gr_id = '$member[mb_id]' ");
$gr_admin = $gr_info['gr_id'];
...
if (G5_IS_MOBILE) {
    ...
    $bookmark_skin_url     = get_skin_url('board/basic/','bookmark');
