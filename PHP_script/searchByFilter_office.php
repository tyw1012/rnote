...
header("Content-Type: text/html; charset=UTF-8");
 ...
 mysqli_set_charset($con,"utf8");
    ...
    $writer_fin = $obj['writer_fin'];

    if($level =='emp'){
        if($selectedSegment=='임대'){
            $sql_query = "select * from g5_write_$boss where board_list = '$selectedOfferingType' and wr_office_permission = 2 AND wr_sale_type = 1 AND wr_sold_out != 1 ";
        }
        else if($selectedSegment=='매매'){
            $sql_query = "select * from g5_write_$boss where board_list = '$selectedOfferingType' and wr_office_permission = 2 AND wr_sale_type = 2 AND wr_sold_out != 1 ";
        }
        else{
            $sql_query = "select * from g5_write_$boss where board_list = '$selectedOfferingType' and wr_office_permission = 2 AND wr_sold_out = 1 ";
        }
    }
    else{
        if($selectedSegment=='임대'){
            $sql_query = "select * from g5_write_$memberID where board_list = '$selectedOfferingType' and wr_office_permission !='' AND wr_sale_type = 1 AND wr_sold_out != 1 ";
        }
        else if($selectedSegment=='매매'){
            $sql_query = "select * from g5_write_$memberID where board_list = '$selectedOfferingType' and wr_office_permission !='' AND wr_sale_type = 2 AND wr_sold_out != 1 ";
        }
        else{
            $sql_query = "select * from g5_write_$memberID where board_list = '$selectedOfferingType' and wr_office_permission !='' AND wr_sold_out = 1 ";
    ...
    if ($selectedSegment =='거래완료'){
        ...
        if(strlen($writer_fin)>0){
            
            $sql_query .= "AND wr_writer LIKE '%$writer_fin%' ";
