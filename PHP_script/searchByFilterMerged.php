...
header("Content-Type: text/html; charset=UTF-8");
 ...
 mysqli_set_charset($con,"utf8");
    ...
    if($level == 'emp'){
        ...
        if($selectedSegment=='임대'){
            $sql_query = "select count(*) as count from g5_write_$memberID where (wr_office_permission='' or wr_office_permission = 2) AND wr_writer = '$memberName' and wr_sold_out != 1 AND wr_sale_type = 1 ";
        }
        else if($selectedSegment=='매매'){
            $sql_query = "select count(*) as count from g5_write_$memberID where (wr_office_permission='' or wr_office_permission = 2) AND wr_writer = '$memberName' and wr_sold_out != 1 AND wr_sale_type = 2 ";
        }
        else{
            $sql_query = "select count(*) as count from g5_write_$memberID where (wr_office_permission='' or wr_office_permission = 2) AND wr_writer = '$memberName' AND wr_sold_out = 1 ";
        }
    }
    ...
    if($level == 'emp'){
        ...
        if($selectedSegment=='임대'){
            $sql_query2 = "select * from g5_write_$memberID where (wr_office_permission='' or wr_office_permission = 2) AND wr_writer = '$memberName' and wr_sold_out != 1 AND wr_sale_type = 1 ";
        }
        else if($selectedSegment=='매매'){
            $sql_query2 = "select * from g5_write_$memberID where (wr_office_permission='' or wr_office_permission = 2) AND wr_writer = '$memberName' and wr_sold_out != 1 AND wr_sale_type = 2 ";
        }
        else{
            $sql_query2 = "select * from g5_write_$memberID where (wr_office_permission='' or wr_office_permission = 2) AND wr_writer = '$memberName' AND wr_sold_out = 1 ";
        }
    }
    ...
    if($selectedSegment =='임대'){
        ...
        if(strlen($areaMax)>0){
            ...
            $sql_query2 .= "AND wr_area_p <= $areaMax ";
        ...
        if(count($wr_rec_sectors)>0){
            ...
            for ( $i = 0,  $size = count($wr_rec_sectors); $i < $size; ++$i){
                ...
                if($i == 0){
                    ...
                    $sql_query2 .= "wr_rec_sectors LIKE '%$sector%' ";
    
                }
                
                if(0 < $i && $i < $size ){
                    $sql_query2 .= "OR wr_rec_sectors LIKE '%$sector%' ";
                }
