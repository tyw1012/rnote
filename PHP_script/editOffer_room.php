...
//  }
    ...
    $contact = $obj['contact'];
    $roomsShouldBeInserted = $obj['roomsShouldBeInserted'];
    ...
    $bld_hasElev= intval($obj['bld_hasElev']);
    $bld_hasParking= intval($obj['bld_hasParking']);
    ...
    bld_datetime_mod = '$bld_datetime_mod' WHERE bld_id = '$bld_id'";
    
    if(checkValidity_bld()){
    $result= mysqli_query($con, $sql_query) or die("Error in Selecting " . mysqli_error($con));

       if($roomsShouldBeInserted){

            $sql_query3 = "DELETE FROM g5_write_$memberID where wr_bld_match_id ='$bld_id'";
