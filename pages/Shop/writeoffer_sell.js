...
export default class writeoffer_sell extends Component {
  ...
  componentWillMount(){
    ...
    this.setState(
      { 
        ...
        wr_seller_contact: params.wr_seller_contact,      
  ...
  _goNext(){
    ...
    if(this.state.wr_seller_contact!=undefined){
      this.setState({
        wr_seller_contact: this.state.wr_seller_contact.replace(/(\d{3})(\d{4})(\d{4})/, "$1-$2-$3"),
    ...
    style={styles.container}>
        ...
        <View>
          ...
          <View style = {styles.row}>    
              ...
              <Text style={styles.itemName}>매도인연락처 <Text style={{color: 'red', fontSize:12, marginTop:5}}> * </Text></Text>
              ...
              underlineColorAndroid="transparent"
              onChangeText= {wr_seller_contact => this.setState({wr_seller_contact})}
              value={this.state.wr_seller_contact}
              onFocus={(event: Event) => {
                // `bind` the function if you're using ES6 classes
              ...
              <TouchableOpacity
                  style={{position:'absolute', top:5, right:0, justifyContent:'center', alignItems:'center', paddingRight:10, paddingBottom:10,}}
                  onPress={()=>{this.setState({wr_seller_contact:''})}}
                  >
                  <Icon 
                  name="md-close-circle"
                  size={20}
...
AppRegistry.registerComponent('writeoffer_sell', () => writeoffer_sell);
