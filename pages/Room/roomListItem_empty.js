...
export default class RoomListItem_empty extends Component{
...
_roomStyle(item){
      ...
      if (item.wr_room_inactive==1){
        ...
        if(item.wr_o_vacant==1){
           return {flex:1,margin:2.5,minWidth:60, height:60,padding:8, borderWidth:1, borderColor:'#3b4db7', backgroundColor:'#3b4db7', justifyContent:'center', alignItems:'center',borderRadius:5}
  ...
  _roomTextStyle(item){
    ...
    if (item.wr_room_inactive==1){
        ...
        if(item.wr_o_vacant == 1){
            ...
            return {fontSize:12, fontWeight:'bold', color:'#fff'}
...
render(){
    ...
    return(
        ...
        <Text style={item.wr_room_inactive==1?{display:'none'}:item.wr_o_vacant == 1?
        {fontSize:11, color:'#ffffff77', marginLeft:2, marginBottom:1, fontWeight:'bold'}:
        {fontSize:11, color:'#c1c1c1', marginLeft:2, marginBottom:1, fontWeight:'bold'}}>
